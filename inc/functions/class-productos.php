...
class Productos {
    ...
    public function getProducts($opcion, $id_rubro, $id_subrubro, $id_grupo, $minamount, $maxamount, $order){
        ...
        $result = [
            ...
            'query' => $query,
            'params' => ($opcion ? 'opcion='. $opcion .'&' : null).(is_array($id_rubro) ? 'id_rubro[]=' . implode('&id_rubro[]=', $id_rubro) : 'id_rubro='.$id_rubro). '&id_subrubro='.$id_subrubro.'&id_grupo='.$id_grupo. (($minamount && $maxamount) ? '&minamount='.$minamount.'&maxamount='.$maxamount : '') . (($order) ? '&order='.$order : '')
        ];
        return $result;
    }
